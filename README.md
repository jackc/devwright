...
This branch implements a reusable **Lima + Ubuntu 24.04 + Codex** environment.
`jack` administers the VM with sudo. Humans and agents both develop as `dev`,
...
Requirements on the host: Lima **2.2+**, Python **3.11+**, and OpenSSH.
...
python3 scripts/vm.py create agent-dev
python3 scripts/vm.py verify agent-dev
python3 scripts/vm.py install-ssh agent-dev

ssh agent-dev         # dev: development, Codex, repositories
ssh agent-dev-admin   # jack: VM administration
```

Create another isolated environment with the same recipe:

```sh
python3 scripts/vm.py create another-dev
python3 scripts/vm.py verify another-dev
python3 scripts/vm.py install-ssh another-dev
```
...
```sh
python3 scripts/vm.py start agent-dev
python3 scripts/vm.py shell agent-dev   # same account as ssh agent-dev
python3 scripts/vm.py admin agent-dev  # administrative session
```
...
forwarding, and connection sharing so a `dev` login cannot reuse a `jack` connection.
...
```sh
python3 scripts/vm.py set-token agent-dev
```
...
```sh
python3 scripts/vm.py configure agent-dev
python3 scripts/vm.py verify agent-dev
```
...
public login keys from `jack`, and `dev`'s empty supplementary group list. Add
...
* Linux protects `jack` and `/root` from `dev`; managed Codex policy additionally
...
* Ubuntu's global AppArmor user-namespace restriction stays enabled. The included
  **VM-only** exception permits `/usr/bin/bwrap` to create namespaces, using an
  unconfined AppArmor profile. This is an explicitly approved compatibility
  exception for the sandbox helper; Codex still applies its own filesystem
  sandbox. It is not a separate AppArmor confinement policy.
* The helper and policy files are root-owned. `jack` has passwordless guest sudo;
...
* Anyone running as `dev` can modify that account's startup files, tools, and
  project code. Do not run such files as `jack` with sudo. Guest policy controls
...
Local checks: `python3 -m unittest discover -s tests -v` and
