...
#   bash users-macos.sh /absolute/path/dev-sandbox [--keep]
...
set -euo pipefail
case "$(uname -s)" in Darwin) base=/private/var/db/dev-sandbox; homes=/Users ;; Linux) base=/var/lib/dev-sandbox; homes=/home ;; *) echo 'Requires macOS or Linux' >&2; exit 1 ;; esac
...
if [ "${1:-}" = --cleanup ]; then
  ...
  binary=${1:?Usage: bash users-macos.sh /absolute/path/dev-sandbox [--keep]}
  case "$binary" in /*) ;; *) binary="$(cd "$(dirname "$binary")" && pwd)/$(basename "$binary")" ;; esac
  test -x "$binary"
  if [ "${2:-}" = --keep ]; then keep=true; fi
  fixture=$(mktemp -d "${TMPDIR:-/tmp}/dev-sandbox-users.XXXXXX")
  run_id="nt$(date +%s)$$"
...
account_exists() { id "dsb-$1" >/dev/null 2>&1; }
as_user() {
  local name=$1; shift
  (cd / && sudo -n -u "dsb-$name" -H env -i HOME="$homes/dsb-$name" USER="dsb-$name" LOGNAME="dsb-$name" PATH=/usr/bin:/bin /bin/sh -c 'cd "$HOME" || exit; exec "$@"' dev-sandbox-acceptance "$@")
}
...
wait_idle() {
  ...
  else uid=$(id -u "dsb-$name") || return 1; fi
  for attempt in $(seq 1 20); do
    ...
    if [ "$(uname -s)" = Darwin ] && [ "$booted" = false ] && printf '%s\n' "$processes" | only_macos_helpers "$uid"; then
      printf 'Stopping macOS background services for recorded fixture dsb-%s (UID %s)\n' "$name" "$uid"
  ...
  printf '%s\n' "$processes" >&2
  echo "Fixture dsb-$name still has processes; close them before retrying cleanup." >&2
  return 1
}
restore_admin_fixtures() {
  ...
  local path
  for path in "$base/users/${names[0]}.json" "/etc/ssh/dev-sandbox/dsb-${names[0]}.keys"; do
...
cleanup() {
  ...
  if [ -f "$fixture/child-pid" ]; then
    ...
    case "$pid" in ''|*[!0-9]*) ;; *)
      if [ "$(ps -o uid= -p "$pid" 2>/dev/null | tr -d ' ')" = "$(id -u "dsb-${names[0]}" 2>/dev/null)" ]; then
        as_user "${names[0]}" /usr/bin/touch "$homes/dsb-${names[0]}/.dev-sandbox-test-stop" || true
      fi ;;
    esac
...
for name in "${names[@]}"; do
  ...
  test "$(as_user "$name" /bin/pwd -P)" = "$homes/dsb-$name"
...
restore_admin_fixtures
keys="/etc/ssh/dev-sandbox/dsb-$a.keys"
...
restore_admin_fixtures
if as_user "$a" /bin/cat "$homes/dsb-$b/private-fixture" 2>/dev/null; then echo 'Cross-account read allowed' >&2; exit 1; fi
if as_user "$b" /bin/sh -c "printf changed > '$homes/dsb-$a/private-fixture'" 2>/dev/null; then exit 1; fi
...
printf '%s\n' "export GH_TOKEN='synthetic-restricted-token'" "export OTHER_API_KEY='synthetic value'" | as_user "$a" /bin/sh -c 'cat > "$HOME/.config/dev-sandbox/credentials.sh"'
...
as_user "$a" /bin/cat "$homes/dsb-$a/.config/dev-sandbox/credentials.sh" > "$fixture/credentials.before"
as_user "$a" /bin/cat "$homes/dsb-$a/.codex/config.toml" > "$fixture/config.before"
cli configure "$a" > "$fixture/configure.log" 2>&1 || { cat "$fixture/configure.log"; exit 1; }
as_user "$a" test -L "$homes/dsb-$a/.bashrc"
as_user "$a" /bin/cat "$homes/dsb-$a/.config/dev-sandbox/credentials.sh" > "$fixture/credentials.after"
as_user "$a" /bin/cat "$homes/dsb-$a/.codex/config.toml" > "$fixture/config.after"
...
as_user "$a" test ! -L "$homes/dsb-$a/.codex/config.toml"
as_user "$a" /bin/cat "$homes/dsb-$a/config-preserve-target" > "$fixture/config-target.after"
...
as_user "$a" /bin/cat "$homes/dsb-$a/.zshenv" > "$fixture/zshenv.before"
...
as_user "$a" /bin/cat "$homes/dsb-$a/.zshenv" > "$fixture/zshenv.malformed"
...
if ssh -F /dev/null -o BatchMode=yes -o ConnectTimeout=5 -o IdentityAgent=none -o IdentitiesOnly=yes -o StrictHostKeyChecking=yes -o HostKeyAlias="user-$a" -o UserKnownHostsFile="$fixture/client/.ssh/dev-sandbox/user/$a/known_hosts" -i "$fixture/wrong-key" "dsb-$a@localhost" true 2> "$fixture/wrong-key.log"; then echo 'Wrong key accepted' >&2; exit 1; fi
grep -q 'Permission denied' "$fixture/wrong-key.log"
# An active account must be refused without terminating its process.
as_user "$a" /bin/sh -c 'echo $$; attempt=0; while [ "$attempt" -lt 120 ] && [ ! -f "$HOME/.dev-sandbox-test-stop" ]; do sleep 1; attempt=$((attempt + 1)); done' > "$fixture/child-pid" &
child_wrapper=$!
...
as_user "$a" /usr/bin/touch "$homes/dsb-$a/.dev-sandbox-test-stop"
