...
git -C "$fixture/repo" worktree add -q --detach "$fixture/worktree"
codex sandbox --include-managed-config -P vm_dev -C "$fixture/worktree" \
  bash -euc '
    fixture=$1
    ...
    git worktree remove "$fixture/worktree"
    test "$(cat feature.txt)" = feature
    test ! -e "$fixture/worktree"
